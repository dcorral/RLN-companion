...
When `service.auth_token` is set, `/companion/transfers`, `/companion/transfers/{id}` and `/companion/openapi.yaml` require `Authorization: Bearer <token>` (scheme case-insensitive) and answer 401 otherwise. It guards `/companion/*` only, never the proxied RLN routes, which stay under RLN's own auth.
